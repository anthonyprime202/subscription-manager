<script lang="ts">
	...
	import { paymentHistoryColumns, pendingPaymentsColumns, type paymentHistoryData, type PendingPaymentsData } from "./columns";
	...
	let historyData = $derived(
		sheetState.paymentSheet
			.map(s => {
				const currentSheet = sheetState.subscriptionSheet.find(su => s.subscriptionNo === su.subscriptionNo)!;
				return {
